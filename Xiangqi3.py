...
def flying_general(x, y, enemy_general_x, enemy_general_y, board, color, raw):
    
    col = get_column(board[y+1:enemy_general_y] if color=="b" else board[enemy_general_y+1:y], x)

    blocked = col.count("_")!=len(col)
               
    if(not blocked):
        if (not is_pinned(x, y, enemy_general_x, enemy_general_y, raw)):
            return True
    return False
...
def get_possible_moves(x, y, board, raw):
    ...
    # For general / king
        ...
        # Handle flying general

        enemy_general_x, enemy_general_y = None, None
        looking_for = "rg" if color == "b" else "bg"

        for iy in range(len(board)):
            for ix in range(len(board[0])):
                if(board[iy][ix]==looking_for):
                    enemy_general_x, enemy_general_y = ix, iy
                    break
            else:
                continue
            break
        
        if(x==enemy_general_x and flying_general(x, y, enemy_general_x, enemy_general_y, board, color, raw)):
            moves.append((enemy_general_x, enemy_general_y))
...
def handle_click(pos):
    global selected_piece, selected_position, current_turn, valid_moves, board, in_check, running
    ...
    if selected_piece is None:
        ...
        if(is_checkmate()):
            if(in_check):
                ...
                draw_endgame_message(f"Checkmate! {'Red' if current_turn == 'b' else 'Black'} wins.")
            else:
                print(f"Stalemate! {'Red' if current_turn == 'b' else 'Black'} wins.")
                draw_endgame_message(f"Stalemate! {'Red' if current_turn == 'b' else 'Black'} wins.")
    ...
    while running:
        ...
        if ai_enabled and (ai_vs_ai or current_turn == ai_color) and not ai_thinking:
            ...
            if board_states[board_str] >= 3:
                print(f"3-fold repetition. {'Red' if current_turn == 'b' else 'Black'} wins.")
                draw_endgame_message(f"3-fold repetition.{'Red' if current_turn == 'b' else 'Black'} wins.")
                running = False
                break
